...
public class PlayerController : MonoBehaviour
{
    ...
    {
        canDash = false; // cooldown Dash
        isDashing = true; // disable Button Left Right
        DashTrail.emitting = true; // turn on Trail

        if (PlayerSpriteRenderer.flipX == true)
            DashDirection = new Vector2(1, 0); // moving to Right
        else
            DashDirection = new Vector2(-1, 0); // moving to Left

        PlayerRigidbody2D.velocity = DashDirection * DashPower; // apply Dash to move Player
        StartCoroutine(PrepareNonLoopAnimation("Player Dash"));// Disable other Loop Animations, then play Dash Anim
        yield return new WaitForSeconds(0.1f); // duration of Dash
        PlayerRigidbody2D.velocity = DashDirection * MoveSpeed; // stop Player sliding after Dash
        isDashing = false; // enable Button Left Right
        DashTrail.emitting = false; // turn off Trail

        yield return new WaitForSeconds(1f); //reset Cooldown Dash
        canDash = true; // can play Dash again now
    }
