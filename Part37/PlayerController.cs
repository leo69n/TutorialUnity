...
public class PlayerController : MonoBehaviour
{
    ...
    IEnumerator PlayerDash()
    {
        ...
        StartCoroutine(PrepareNonLoopAnimation("Player Dash"));
        yield return new WaitForSeconds(0.1f);
        PlayerRigidbody2D.velocity = DashDirection * MoveSpeed; 
        isDashing = false;
        //..end Dash
    ...
    {
        PlayerRigidbody2D.velocity = new Vector2( (MoveVector.x * MoveSpeed) + MovingPlatformVelocityX, PlayerRigidbody2D.velocity.y);
        isMoving = true;
    ...
    //..
    //..Begin Tutorial
    public float MovingPlatformVelocityX = 0;
    private void OnCollisionStay2D(Collision2D collision)
    {
        if (collision.gameObject.tag == "MovingPlatform")
        {
            var MovingPlatformRigidbody2DComponent = collision.gameObject.GetComponent<Rigidbody2D>();
            MovingPlatformVelocityX = MovingPlatformRigidbody2DComponent.velocity.x;
        }
    }
